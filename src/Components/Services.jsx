    ...
    return (
        <ParallaxProvider>
            ...
            >
                <div className='flex justify-center items-center flex-col w-full'>
                    <div className='text-3xl font-semibold mb-5'>When it comes to</div>
                    <div className='services-service text-4xl lg:text-7xl md:text-5xl w-full h-full flex flex-col justify-around items-center font-bold tracking-tighter'>
                        {services.map((service, index) => (
                            <Parallax key={service.name} translateX={index % 2 === 0 ? [25, -20] : [-25, 20]}>
                                <div
                                    className={`service-item px-5 py-2 mx-5 ${hoveredService && hoveredService.name !== service.name ? 'fade' : ''
                    ...
                    {hoveredService && hoveredService.name === 'MARKETING' ? <div className='text-3xl font-semibold mb-5'>we are the storytellers.</div> : null}
                    {hoveredService && hoveredService.name === 'BRANDING' ? <div className='text-3xl font-semibold mb-5'>we build legacies.</div> : null}

                    {hoveredService && hoveredService.name === 'DESIGN' ? <div className='text-3xl font-semibold mb-5'>we speak the language of visuals.</div> : null}

                    {hoveredService && hoveredService.name === 'DEVELOPMENT' ? <div className='text-3xl font-semibold mb-5'> we bridge the gap between ideas and reality.</div> : null}

                    {hoveredService && hoveredService.name === 'DIGITAL' ? <div className='text-3xl font-semibold mb-5'> we future proof your brand.</div> : null}
